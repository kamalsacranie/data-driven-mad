...
#show heading: it => {
  ...
  let post_heading_rule = align(horizon)[#line(
    stroke: 0.5pt,
    length: 100% - spacing + heading_offset - measure(it.body).width - 0.1em
  )]
  block(
...
While working as an Equity Analyst, I realised my passion lay elsewhere---no
normal financial analyst writes Neovim plugins! I then pursued a Master's in AI
& Computer Science, where I not only developed a novel programming language for
Mathematical Linear Programming but also graduated Top of Class. I now work on
Lloyds Banking Group's flagship Pension App.
...
**Languages I love to use:** OCaml, Haskell, Kotlin, Lua

**Language I put up with:** C++, Typescript, Python

**Nerdy interests:** I use Neovim and love writing compilers
:::
...
def experience(entry: WorkExp):
  return Markdown(qmd_table(surround_tag("table")(f"""
    <tr>
      {table_data(
        f"<strong>{qmd_raw(entry.employer)}</strong>", align="left", colspan=2
      )}
      {table_data(
    ...
    <tr>
      {table_data(ulist([qmd_raw(e) for e in entry.data]), colspan=4)}
    </tr>
""")))
...
|                                                     |
+-----------------------------------------------------+
| **Fun fact:** I have a diploma in classical singing |
+-----------------------------------------------------+
